...
# --- YENİ EKLENEN GEZEGEN RAPORLARI ---
@router.post("/report/mercury-sign", summary="Merkür Burcu Raporu", description="Merkür'ün burcunu ve zihinsel işleyişle ilgili yorumunu döndürür.")
def get_mercury_sign_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    planets_map = {p['planet']: p for p in natal_data['planets']}
    interpretations = load_interpretations("mercury_in_signs.json")
    planet_data = _get_planet_from_map(planets_map, "Mercury")
    interpretation = interpretations.get(planet_data['sign'], "Bu burç için yorum bulunamadı.")
    return {"report_type": "Mercury in Sign", **planet_data, "interpretation": interpretation}

@router.post("/report/venus-sign", summary="Venüs Burcu Raporu", description="Venüs'ün burcunu ve ilişkilerle ilgili yorumunu döndürür.")
def get_venus_sign_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    planets_map = {p['planet']: p for p in natal_data['planets']}
    interpretations = load_interpretations("venus_in_signs.json")
    planet_data = _get_planet_from_map(planets_map, "Venus")
    interpretation = interpretations.get(planet_data['sign'], "Bu burç için yorum bulunamadı.")
    return {"report_type": "Venus in Sign", **planet_data, "interpretation": interpretation}

@router.post("/report/mars-sign", summary="Mars Burcu Raporu", description="Mars'ın burcunu ve eylem tarzıyla ilgili yorumunu döndürür.")
def get_mars_sign_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    planets_map = {p['planet']: p for p in natal_data['planets']}
    interpretations = load_interpretations("mars_in_signs.json")
    planet_data = _get_planet_from_map(planets_map, "Mars")
    interpretation = interpretations.get(planet_data['sign'], "Bu burç için yorum bulunamadı.")
    return {"report_type": "Mars in Sign", **planet_data, "interpretation": interpretation}

@router.post("/report/jupiter-sign", summary="Jüpiter Burcu Raporu", description="Jüpiter'in burcunu ve şans alanlarıyla ilgili yorumunu döndürür.")
def get_jupiter_sign_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    planets_map = {p['planet']: p for p in natal_data['planets']}
    interpretations = load_interpretations("jupiter_in_signs.json")
    planet_data = _get_planet_from_map(planets_map, "Jupiter")
    interpretation = interpretations.get(planet_data['sign'], "Bu burç için yorum bulunamadı.")
    return {"report_type": "Jupiter in Sign", **planet_data, "interpretation": interpretation}

@router.post("/report/saturn-sign", summary="Satürn Burcu Raporu", description="Satürn'ün burcunu ve hayat dersleriyle ilgili yorumunu döndürür.")
def get_saturn_sign_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    planets_map = {p['planet']: p for p in natal_data['planets']}
    interpretations = load_interpretations("saturn_in_signs.json")
    planet_data = _get_planet_from_map(planets_map, "Saturn")
    interpretation = interpretations.get(planet_data['sign'], "Bu burç için yorum bulunamadı.")
    return {"report_type": "Saturn in Sign", **planet_data, "interpretation": interpretation}
# --- BİTTİ ---
...
def get_planets_in_houses_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    ...
    for planet_data in natal_data['planets']:
        if planet_data['planet'] in planets_to_interpret:
            ...
            interpretation = interpretations.get(house_number, "Bu gezegen/ev kombinasyonu için yorum bulunamadı.")
...
def get_balance_report(natal_data: Dict[str, Any] = Depends(get_natal_data_dependency)):
    ...
    return { "report_type": "Chart Balance (Insights)", "data": balance_data }